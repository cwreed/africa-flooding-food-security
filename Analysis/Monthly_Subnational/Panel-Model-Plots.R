...
make_plot <- function(summary_tibble, name, sig_only=FALSE, facet_by_granger=FALSE){
  ...
  summary_tibble <- summary_tibble %>%
                    rowwise() %>%
                    mutate(model_color = case_when(model %in% c('East Africa', 'Southeast Africa', 'West Africa', 'All-Africa Full') ~ "#000000",
...
plot_1_tidy$p_fdr <- all_africa_p_fdr

plot_1 <- make_plot(plot_1_tidy, "All-Africa Random Effects Model: Full v. Granger-filtered data", sig_only=F, facet_by_granger = F)
...
plot_3a_tidy <- data.frame(rbind(regional_tidies[["West Africa"]],
                     country_tidies[["Niger"]],
                     country_tidies[["Nigeria"]],
                     country_tidies[["Mali"]],
                     country_tidies[["Burkina Faso"]],
                     country_tidies[["Chad"]],
                     regional_granger_tidies[["West Africa"]],
...
plot_3a <- make_plot(plot_3a_tidy, "West Africa Random Effects Models", sig_only=T, facet_by_granger = T)

plot_4a_tidy <- rbind(regional_tidies[["Southeast Africa"]],
                     country_tidies[["Mozambique"]],
                     country_tidies[["Malawi"]],
                     country_tidies[["Zambia"]],
                     country_tidies[["Zimbabwe"]],
                     regional_granger_tidies[["Southeast Africa"]],
...
plot_4b <- make_plot(plot_4b_tidy, "Southeast Africa Random Effects Models", sig_only=T, facet_by_granger = F)
